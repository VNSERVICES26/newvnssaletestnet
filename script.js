...
async function loadBalances() {
  ...
  const vns = new web3.eth.Contract(vnsAbi, vnsTokenAddress);

  const usdtBal = await usdt.methods.balanceOf(userAccount).call();
  const vnsBal = await vns.methods.balanceOf(sellerAddress).call();

  document.getElementById("usdtBalance").innerText = web3.utils.fromWei(usdtBal, 'ether') + " USDT";
  document.getElementById("sellerVnsBalance").innerText = (vnsBal / 1e8) + " VNS";
}
...
document.getElementById("buyBtn").onclick = async () => {
  ...
  try {
    // ✅ USDT Approval
    const allowance = await usdt.methods.allowance(userAccount, presaleContractAddress).call();
    if (Number(allowance) < Number(usdtAmount)) {
    ...
    // ✅ Buying VNS Tokens
    buyStatus.innerText = "⏳ Buying VNS...";
    await presale.methods.buyTokens(usdtAmount).send({
      from: userAccount,
      gas: 300000 // Set higher gas limit
    });

    buyStatus.innerText = "✅ Buy Successful!";
    loadBalances();
  } catch (err) {
    console.error("Buy Error", err);
    buyStatus.innerText = "❌ Buy Failed: " + (err?.message || "Unknown error");
  }
};

function copyAddress() {
  const address = document.getElementById("vnsAddress").innerText;

...
async function copyAddress() {
  const text = document.getElementById("vnsAddress").textContent;
...
async function loadDetails() {
  ...
  document.getElementById("usdtBalance").textContent = web3.utils.fromWei(usdtBal, 'ether');
...
async function approveTokens() {
  try {
    const vns = new web3.eth.Contract(vnsAbi, vnsTokenAddress);
    const amount = (100000 * 1e8).toString(); // 100,000 VNS with 8 decimals

    await vns.methods.approve(presaleContractAddress, amount).send({ from: userAccount });

    document.getElementById("approveStatus").textContent = "✅ Approved 100000 VNS for Presale";
  } catch (err) {
    console.error(err);
    document.getElementById("approveStatus").textContent = "❌ Approval Failed";
  }
}
...
async function buyVNS() {
  try {
    ...
    console.error(err);
    document.getElementById("buyStatus").textContent = "❌ Buy Failed";
  }
}

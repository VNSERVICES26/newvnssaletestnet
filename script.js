// Contract ABIs और configuration (आपके मूल कोड के अनुसार)
...
async function init() {
    await initWeb3();
    await initContracts(); // यहाँ त्रुटि हो रही थी
    await loadContractData();
    setupEventListeners();
}

// नया जोड़ा गया फंक्शन
async function initContracts() {
    if (!web3) {
        showNotification("Web3 इनिशियलाइज़ नहीं हुआ", "error");
        return;
    }
    ...
    try {
        ...
        // टोकन डेसीमल्स प्राप्त करें
        vnsDecimals = await vnsContract.methods.decimals().call();
        usdtDecimals = await usdtContract.methods.decimals().call();
        
        showNotification("कॉन्ट्रैक्ट्स सफलतापूर्वक लोड हुए", "success");
    } catch (error) {
        console.error("कॉन्ट्रैक्ट इनिशियलाइज़ेशन त्रुटि:", error);
        showNotification("कॉन्ट्रैक्ट्स लोड नहीं हो पाए", "error");
    }
}

...
const sellerAddress = "0xab75520a1C19483cbA1Fbd2c74366164aaA3FD41"; 
...
async function copyTokenAddress() {
  const addr = document.getElementById("tokenAddress").textContent;
  await navigator.clipboard.writeText(addr);
  alert("VNS Token address copied!");
}

async function loadBalances() {
  const usdt = new web3.eth.Contract(usdtAbi, usdtAddress);
  const vns = new web3.eth.Contract(vnsAbi, vnsAddress);

  const sellerBal = await vns.methods.balanceOf(sellerAddress).call();
...
async function approveUSDT() {
  ...
  if (amount < 10) return alert("🚫 Minimum buy is 10 VNS");

  const usdt = new web3.eth.Contract(usdtAbi, usdtAddress);
  const vnsToBuy = parseFloat(amount);
  const usdtPerVns = 1; // Change if needed
  const usdtAmount = (vnsToBuy * usdtPerVns * 1e18).toString(); // 18 decimals now

  try {
    await usdt.methods.approve(presaleContractAddress, usdtAmount).send({ from: userAccount });
    document.getElementById("status").textContent = "✅ USDT Approved";
  } catch (err) {
    console.error(err);
    document.getElementById("status").textContent = "❌ Approval Failed: " + err.message;
  }
}
...
async function buyVNS() {
  ...
  const vnsToBuy = parseFloat(amount);
  const usdtPerVns = 1; // Change if needed
  const usdtAmount = (vnsToBuy * usdtPerVns * 1e18).toString(); // 18 decimals

  try {
    await presale.methods.buyTokens(usdtAmount).send({ from: userAccount });
    document.getElementById("status").textContent = "✅ VNS Buy Successful!";
    loadBalances();
  } catch (err) {
    console.error(err);
    document.getElementById("status").textContent = "❌ Buy Failed: " + err.message;
  }
}
